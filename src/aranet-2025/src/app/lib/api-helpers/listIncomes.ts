...
import { aranet_income_item, PrismaClient } from "@/generated/prisma";
import { INCOME_VALID_FIELDS, INCOME_TEXT_FIELDS } from "@/app/data/income";
import { searchWhere } from "./utils";

const filterByValidFields = (filter: SearchDTO) => {
  return INCOME_VALID_FIELDS.includes(filter.field.toLowerCase()) || filter.field === 'all';
};
...
export const listIncomes = async (
  ...
  search: SearchDTO[],
  filters: string[],
): Promise<ListResponse<aranet_income_item>> => {
  search = search.filter(filter => filterByValidFields(filter))
  logDebug(`GET /api/income_item - Búsquedas encontradas: ${JSON.stringify(search)}`);
  ...
  try {
    ...
    searchWhere(where, search, INCOME_TEXT_FIELDS);
    logDebug(`GET /api/income - Where generado: ${JSON.stringify(where)}`);

    const orderBy: Record<string, string> = {};
    orderBy[sortField] = sortDir;
    const nbItems = await prisma.aranet_income_item.count({ where });
    if (size === -1) {
      size = nbItems;
      page = 1;
    }
    ...
    const income_items = await prisma.aranet_income_item.findMany({
      where,
      orderBy,
      skip: start,
      take: size,
    ...
    logError(`Error GET /api/income: ${err}`);
    return {
      statusCode: 500,
