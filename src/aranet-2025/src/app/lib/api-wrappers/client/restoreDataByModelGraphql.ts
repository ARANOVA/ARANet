import { RESTORE_QUERIES } from "@/graphql/queries";
import { SingleResponse } from "@aranova/aranova-react-ui";
import { logError } from "../../logger";

export const restoreDataByModelGraphql = async (
  model: string,
  ids: number[],
  ...
  const query = RESTORE_QUERIES.restore;
  if (!query) return { statusCode: 404, error: 'Query not found' };
  try {
    const res = await fetch('/api/graphql', {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ query, variables: { model, ids } }),
    });
    ...
    logError(`POST (RESTORE) /api/graphql ${model}: ${err}`);
    return {
      statusCode: 500,
      error: `${err}`,
    };
